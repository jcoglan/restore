...
var Restore = function(options) {
  ...
  this.remotestorageServer = new RemotestorageServer('draft-dejong-remotestorage-01', {
    _data: {},
    get: function(key, cb) {
      console.log('querying token', key);
      self._store.queryToken(key, cb);
    },
    ...
    get: function(key, cb) {
      ...
      self._store.get(key, cb);
    },
    set: function(key, value, cb) {
      if (value === undefined) {
        self._store.delete(key, cb);
      } else {
        self._store.put(key, value, cb);
      }
    }
  }); 
...
Restore.prototype.dispatch = function(request, response) {
  ...
  if (match) {
    ...
    this.remotestorageServer.storage(request, response);
  }
