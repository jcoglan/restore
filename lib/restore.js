...
Restore.prototype.handle = function(request, response) {
  ...
  if (method === 'get' && uri.pathname === '/.well-known/host-meta')
    return this.renderXRD(response, 'host-meta.xml', {
      template_url: 'http://localhost/webfinger/{uri}'
    });
  ...
  if (match) {
    ...
    if (method === 'get')
      return this._store.get(token, username, category, key, function(error, item) {
        var status = error ? 401 : item ? 200 : 404;
        if (item) {
    ...
    if (method === 'put')
      return this.parseBody(request, function(error, value) {
        ...
        self._store.put(token, username, category, key, type, value, function(error, created) {
          var status = error ? 401 : created ? 201 : 200;
          response.writeHead(status, headers);
          response.end();
        });
      });
    ...
    if (method === 'delete')
      return this._store.delete(token, username, category, key, function(error, deleted) {
        var status = error ? 401 : deleted ? 200 : 404;
        response.writeHead(status, headers);
        response.end();
      });
  }
