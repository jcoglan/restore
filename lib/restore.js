...
Restore.prototype.handle = function(request, response) {
  ...
  match = uri.pathname.match(/^\/webfinger\/xrd\/acct:(.*)$/);
  if (method === 'get' && match)
  ...
  if (method === 'post' && uri.pathname === '/auth')
    return this.parseBody(request, function(error, params) {
      var permissions = self.parseScope(params.scope);
  ...
  match = uri.pathname.match(/^\/data\/([^\/]+)\/((?:public\/)?[^\/]+)(\/.*)$/);
  if (match) {
    ...
    if (method === 'options') {
      ...
      headers['Access-Control-Allow-Headers'] = 'Authorization, Content-Length, Content-Type, Origin, X-Requested-With';
      response.writeHead(200, headers);
      return response.end();
    }
    if (method === 'get')
      return this._store.get(token, username, category, key, function(error, item) {
        ...
        if (item instanceof Array) {
          ...
          headers['Content-Type']  = item.type || 'text/plain';
          headers['Last-Modified'] = item.modified.toGMTString();
        }
        response.writeHead(status, headers);
        if (item) response.write(item.value);
        response.end();
      });
    
    if (method === 'put')
      return this.parseBody(request, function(error, value) {
        var type = (request.headers['content-type'] || '').split(/\s*;\s*/)[0];
        self._store.put(token, username, category, key, type, value, function(error, created, modified) {
          var status = error ? error.status : created ? 201 : 200;
          if (modified) headers['Last-Modified'] = modified.toGMTString();
          response.writeHead(status, headers);
          response.end();
        });
      });
    ...
    if (method === 'delete')
      return this._store.delete(token, username, category, key, function(error, deleted) {
...
Restore.prototype.parseScope = function(scope) {
