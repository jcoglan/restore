    ...
    self.getCurrentState(client, dataKey, version, function(error, current, mtime, exists) {
      ...
      async.waterfall([
        function(next) {
          ...
          async.map(core.indexed(children), function(childList, callback) {
            var multi = client.multi();
            childList.value.forEach(function(child) {
              multi.hget(prefix + remaining[childList.index] + child, 'modified');
            });
            multi.exec(callback);
          }, function(error, mtimes) {
            next(error, index, empty, remaining, children, mtimes);
          });
        }, function(index, empty, remaining, children, mtimes, next) {
          var ancestors = {},
              multi     = client.multi(),
              item;
          ...
          if (remaining.length > 0) {
            ...
            mtimes[0].splice(children[0].indexOf(item), 1);

            remaining.forEach(function(dir, i) {
              ancestors[dir] = Math.max.apply(Math, mtimes[i]);

              if (i + 1 < remaining.length) {
                item = query[query.length - empty.length - 2 - i];
                mtimes[i + 1][children[i + 1].indexOf(item)] = ancestors[dir];
              }
            });

            for (var dir in ancestors)
              multi.hset(prefix + dir, 'modified', ancestors[dir].toString());
          }
