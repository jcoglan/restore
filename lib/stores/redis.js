...
RedisStore.prototype.permissionPath = function(username, token, category) {
  ...
  if (category === undefined) return root + '|clients|' + token + '|categories';
  category = category.replace(/^\/?/, '/').replace(/\/?$/, '/');
  return root + '|clients|' + token + '|categories|' + category + '|permissions';
};
...
RedisStore.prototype.permissions = function(username, token, callback) {
  ...
  client.smembers(self.permissionPath(username, token), function(error, scopes) {
