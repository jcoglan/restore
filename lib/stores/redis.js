  ...
  this.checkToken(token, username, category, 'r', function(error) {
    ...
    if (isdir) {
      client.smembers(key + '|children', function(error, children) {
        var listing = [], n = children.length;
        if (n === 0) return callback(null, listing);
        children.sort().forEach(function(child) {
          client.hget(key + child, 'modified', function(error, modified) {
            listing.push({name: child, modified: new Date(parseInt(modified, 10))});
            if (listing.length === n) callback(null, listing);
          });
        });
      });
    } else {
      client.hgetall(key, function(error, hash) {
        if (hash) hash.modified = new Date(parseInt(hash.modified));
        callback(error, hash);
      });
    }
  });
...
RedisStore.prototype.put = function(token, username, category, path, type, value, callback) {
  ...
  this.checkToken(token, username, category, 'w', function(error) {
    ...
    for (var i = 0, n = query.length; i < n; i++) {
      key = 'users|' + username + '|categories|' + category + '|data|' + query.slice(0,i+1).join('');
      client.hset(key, 'modified', modified.getTime());
      client.sadd(key + '|children', query[i+1] || name);
    }
    
    key = 'users|' + username + '|categories|' + category + '|data|' + path;
