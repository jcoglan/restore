...
RedisStore.prototype.authorize = function(clientId, username, permissions, callback) {
  ...
  multi.hget(this.authPath(username), 'key', function(error, key) {
    ...
    cipher.encrypt(clientId, callback);
  });
  multi.exec();
};
...
RedisStore.prototype.authorizations = function(username, callback) {
  ...
  client.smembers(this.permissionPath(username), function(error, clients) {
    async.forEach(clients, function(id, done) {
      authorizations[id] = {};
      client.smembers(self.permissionPath(username, id), function(error, scopes) {
        async.forEach(scopes, function(dir, next) {
...
RedisStore.prototype.get = function(username, path, callback) {
  var self   = this,
      isdir  = /\/$/.test(path),
      client = this.redisFor(username);
  ...
  if (isdir) {
    this._lock(username, function(release) {
      client.smembers(key + '|children', function(error, children) {
        if (children.length === 0) return callback(null, null);
...
RedisStore.prototype.put = function(username, path, type, value, callback) {
  var self   = this,
...
RedisStore.prototype.delete = function(username, path, callback) {
  var self   = this,
      query  = core.parsePath(path),
      name   = query.pop(),
      client = this.redisFor(username);
