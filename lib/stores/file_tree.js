...
FileTree.prototype.keyPath = function(key) {
  var b = new Buffer(key);
  return path.join(this._dir, '/', b.toString('hex'));
}
FileTree.prototype.getItem = function(key, callback) {
  fs.readFile(this.keyPath(key), callback);
};
FileTree.prototype.putItem = function(key, value, callback) {
  fs.writeFile(this.keyPath(key), value, callback);
};
FileTree.prototype.deleteItem = function(key, callback) {
  fs.unlink(this.keyPath(key), callback);
};
