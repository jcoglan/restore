...
exports.queryToken = function(username, token, callback) {
  this.permissions(username, token, function(error, scopes) {
    var path, ret = [];
    if (error) {
      callback(error);
    } else {
      for (path in scopes) {
        ret.push(path + ':' + (scopes[path].indexOf('w') === -1 ? 'r' : 'rw'));
        ret.push('/public' + path + ':' + (scopes[path].indexOf('w') === -1 ? 'r' : 'rw'));
      }
      callback(error, ret);
    }
  });
