...
FileStore.prototype.get = function(token, username, category, path, callback) {
  ...
  this.checkToken(token, username, category, 'r', function(error) {
    ...
    self.readFile(self.dataPath(username, category), function(error, json) {
      if (error) return callback(null, name === '' ? [] : null);
      ...
      if (name === '') {
        ...
        return callback(null, Object.keys(dir.children).sort().map(function(name) {
          var filename = name;
          if (dir.children[name].children) filename += '/';
          return {name: filename, modified: new Date(dir.children[name].modified)};
        }));
      }
      
      if (!dir) return callback(null, null);

      var record = dir.children[name];
