...
JS.Test.describe("Storage", function() { with(this) {
  ...
  before(function() { with(this) {
    ...
    stub(store, "queryToken").given("a_token").yields([null, [
      "zebcoe@local.dev/public/locog/:rw",
      "zebcoe@local.dev/locog/:rw",
      "zebcoe@local.dev/public/books/:r",
      "zebcoe@local.dev/books/:r",
      "zebcoe@local.dev/public/statuses/:rw",
      "zebcoe@local.dev/statuses/:rw",
      "zebcoe@local.dev/public/deep/dir/:rw",
  ...
  describe("GET", function() { with(this) {
    ...
    describe("when a valid access token is used", function() { with(this) {
      ...
      it("asks the store for the item", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/seats").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null, item.modified])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/locog/seats").yielding([null, item.type])
        get( "/storage/zebcoe@local.dev/locog/seats", {} )
      }})

      it("asks the store for items containing dots", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/seats.gif").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats.gif").yielding([null, item.modified])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/locog/seats.gif").yielding([null, item.type])
        get( "/storage/zebcoe@local.dev/locog/seats.gif", {} )
      }})

      it("asks the store for a deep item", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/deep/dir/value").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/deep/dir/value").yielding([null, item.modified])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/deep/dir/value").yielding([null, item.type])
        get( "/storage/zebcoe@local.dev/deep/dir/value", {} )
      }})

      it("passes the path literally to the store", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/a%2Fpath").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/a%2Fpath").yielding([null, item.modified])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/locog/a%2Fpath").yielding([null, item.type])
        get( "/storage/zebcoe@local.dev/locog/a%2Fpath", {} )
      }})

      it("asks the store for a directory listing", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/").yielding([null, emptyDir.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/").yielding([null, emptyDir.modified])
        get( "/storage/zebcoe@local.dev/locog/", {} )
      }})

      it("asks the store for a deep directory listing", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/deep/dir/").yielding([null, emptyDir.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/deep/dir/").yielding([null, emptyDir.modified])
        get( "/storage/zebcoe@local.dev/deep/dir/", {} )
      }})

      it("asks the store for a root listing", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/").yielding([null, emptyDir.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/").yielding([null, emptyDir.modified])
    ...
    describe("when an invalid access token is used", function() { with(this) {
      ...
      it("asks the store for a public item", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/public/locog/seats").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/public/locog/seats").yielding([null, item.modified])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/public/locog/seats").yielding([null, item.type])
    ...
    describe("when the store returns an item", function() { with(this) {
      ...
      it("returns the value in the response", function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/seats").yielding([null, item.value])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null, item.modified.toString()])
        expect(store, "getItem").given("contentType:zebcoe@local.dev/locog/seats").yielding([null, item.type])
        get( "/storage/zebcoe@local.dev/locog/seats", {} )
        check_status( 200 )
        check_header( "Access-Control-Allow-Origin", "*" )
    ...
    describe("when the store returns a directory listing", function() { with(this) {
      before(function() { with(this) {
        ...
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/seats/").yielding([null, items])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats/").yielding([null, modified2])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats/bla").yielding([null, modified1])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats/bar/").yielding([null, modified2])
      }})
    ...
    describe("when the store returns an empty directory listing", function() { with(this) {
      before(function() { with(this) {
        ...
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/seats/").yielding([null, items])
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats/").yielding([null, modified])
      }})
    ...
    describe("when the item does not exist", function() { with(this) {
      before(function() { with(this) {
        header( "Authorization", "Bearer a_token" )
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null, undefined])
      }})
  ...
  describe("PUT", function() { with(this) {
    describe("when a valid access token is used", function() { with(this) {
      ...
      it("tells the store to save the given value", function() { with(this) {
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null, '123'])
        
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/").yielding([null, undefined])
        expect(store, "getItem").given("content:zebcoe@local.dev/").yielding([null, undefined])
        
        expect(store, "putItem").given("content:zebcoe@local.dev/locog/seats", buffer("a value")).yielding([null])
        expect(store, "putItem").given("content:zebcoe@local.dev/locog/", {seats: true}).yielding([null])
        expect(store, "putItem").given("content:zebcoe@local.dev/", {'locog/': true}).yielding([null])
        
        expect(store, "putItem").given("revision:zebcoe@local.dev/locog/seats", '9e4647f796987297ce25c638aa6797954b40b730').yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/locog/", '17e3a1238e9efb98de86d2f3313e6123445a6088').yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/", '453ec93e4a3cd34a3a9b970cbe214f2376292b80').yielding([null])
        
        expect(store, "putItem").given("contentType:zebcoe@local.dev/locog/seats", 'text/plain').yielding([null])
        
        put( "/storage/zebcoe@local.dev/locog/seats", "a value" )
      }})

      it("tells the store to save a public value", function() { with(this) {
        expect(store, "getItem").given("revision:zebcoe@local.dev/public/locog/seats").yielding([null, '123'])
        
        expect(store, "getItem").given("content:zebcoe@local.dev/public/locog/").yielding([null, undefined])
        expect(store, "getItem").given("content:zebcoe@local.dev/public/").yielding([null, undefined])
        expect(store, "getItem").given("content:zebcoe@local.dev/").yielding([null, undefined])
        
        expect(store, "putItem").given("content:zebcoe@local.dev/public/locog/seats", buffer("a value")).yielding([null])
        expect(store, "putItem").given("content:zebcoe@local.dev/public/locog/", {seats: true}).yielding([null])
        expect(store, "putItem").given("content:zebcoe@local.dev/public/", {'locog/': true}).yielding([null])
        expect(store, "putItem").given("content:zebcoe@local.dev/", {'public/': true}).yielding([null])
        
        expect(store, "putItem").given("revision:zebcoe@local.dev/public/locog/seats", '9e4647f796987297ce25c638aa6797954b40b730').yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/public/locog/", '17e3a1238e9efb98de86d2f3313e6123445a6088').yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/public/", '453ec93e4a3cd34a3a9b970cbe214f2376292b80').yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/", '7a5669083f58caaa0fda960d1d1c3b500e9d410c').yielding([null])
        
        expect(store, "putItem").given("contentType:zebcoe@local.dev/public/locog/seats", 'text/plain').yielding([null])
      ...
      it("tells the store to save a value conditionally based on If-Match", function() { with(this) {
        expect(store, "getItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null, modifiedTimestamp])
        ...
        expect(store, "putItem").given("contentType:zebcoe@local.dev/locog/seats", 'text/plain').yielding([null])
        //expect(store, "putItem").given("zebcoe", "/locog/seats", "text/plain", buffer("a value"), modifiedTimestamp).yielding([null])
    ...
    describe("when the store says there was a version conflict creating", function() { with(this) {
      before(function() { with(this) {
        header( "Authorization", "Bearer a_token" )
        stub(store, "putItem").yields([null, false, 1347016875231, true])
      }})
  ...
  describe("DELETE", function() { with(this) {
    ...
    describe("when deletion is successful", function() { with(this) {
      before(function() { with(this) {
        expect(store, "getItem").given("content:zebcoe@local.dev/locog/").yielding([null, { seats: true } ])
        expect(store, "getItem").given("content:zebcoe@local.dev/").yielding([null, { 'locog/': true } ])
        
        expect(store, "deleteItem").given("content:zebcoe@local.dev/locog/seats").yielding([null])
        expect(store, "deleteItem").given("revision:zebcoe@local.dev/locog/seats").yielding([null])
        expect(store, "deleteItem").given("contentType:zebcoe@local.dev/locog/seats").yielding([null])
        
        expect(store, "putItem").given("content:zebcoe@local.dev/locog/", {}).yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/locog/", 'bf21a9e8fbc5a3846fb05b4fa0859e0917b2202f').yielding([null])
        
        expect(store, "putItem").given("content:zebcoe@local.dev/", {}).yielding([null])
        expect(store, "putItem").given("revision:zebcoe@local.dev/", 'bf21a9e8fbc5a3846fb05b4fa0859e0917b2202f').yielding([null])
      }})
